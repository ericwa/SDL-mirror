...
static BOOL WIN_RectFromDPIUnaware_Win10Anniv(_THIS, const RECT rect_unaware, LPRECT rectOut)
{
    ...
    if (videodata->SetThreadDpiAwarenessContext) {
        ...
        HRESULT result;
        HMONITOR hmon;
        MONITORINFO moninfo_unaware, moninfo;
        DPI_AWARENESS_CONTEXT previousDpiContext;

        previousDpiContext = videodata->SetThreadDpiAwarenessContext(DPI_AWARENESS_CONTEXT_UNAWARE);
        if (previousDpiContext == NULL)
            return FALSE;

        hmon = MonitorFromRect(&rect_unaware, MONITOR_DEFAULTTONEAREST);

        /* get DPI-unaware monitor rect */

        moninfo_unaware.cbSize = sizeof(moninfo_unaware);
        ok = GetMonitorInfo(hmon, &moninfo_unaware);

        /* switch back to DPI-aware (or whatever the previous awareness was) */

        videodata->SetThreadDpiAwarenessContext(previousDpiContext);

        if (!ok)
            return FALSE;

        /* get DPI-aware monitor rect */

        moninfo.cbSize = sizeof(moninfo);
        ok = GetMonitorInfo(hmon, &moninfo);
        if (!ok)
            return FALSE;

        result = videodata->GetDpiForMonitor(hmon, MDT_EFFECTIVE_DPI, &xdpi, &ydpi);
        if (result != S_OK)
            return FALSE;

        /* convert rect to DPI-aware */

        xofs = rect_unaware.left - moninfo_unaware.rcMonitor.left;
        yofs = rect_unaware.top - moninfo_unaware.rcMonitor.top;
        xofs = MulDiv(xofs, xdpi, 96);
        yofs = MulDiv(yofs, ydpi, 96);

        w = MulDiv(rect_unaware.right - rect_unaware.left, xdpi, 96);
        h = MulDiv(rect_unaware.bottom - rect_unaware.top, ydpi, 96);

        rectOut->left = moninfo.rcMonitor.left + xofs;
        rectOut->top = moninfo.rcMonitor.top + yofs;
        rectOut->right = rectOut->left + w;
        rectOut->bottom = rectOut->top + h;

        return TRUE;
    }
    /* System DPI aware version */
...
static BOOL WIN_RectToDPIUnaware_Win10Anniv(_THIS, const RECT rect, LPRECT rectOut)
{
    ...
    if (videodata->SetThreadDpiAwarenessContext) {
        ...
        /* convert rect to DPI-unaware */

        xofs = rect.left - moninfo.rcMonitor.left;
        yofs = rect.top - moninfo.rcMonitor.top;
        xofs = MulDiv(xofs, 96, xdpi);
        yofs = MulDiv(yofs, 96, ydpi);

        w = MulDiv(rect.right - rect.left, 96, xdpi);
        h = MulDiv(rect.bottom - rect.top, 96, ydpi);

        rectOut->left = moninfo_unaware.rcMonitor.left + xofs;
        rectOut->top = moninfo_unaware.rcMonitor.top + yofs;
        rectOut->right = rectOut->left + w;
        rectOut->bottom = rectOut->top + h;
